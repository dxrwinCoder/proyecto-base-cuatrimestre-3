...
async def crear_miembro(db: AsyncSession, data: dict):
    try:
        ...
        existe = await db.execute(
            select(Miembro).where(Miembro.correo_electronico == data["correo_electronico"])
        )
        if existe.scalar_one_or_none():
            logger.warning(f"Intento de crear miembro con correo existente: {data['correo_electronico']}")
...
async def obtener_miembro(db: AsyncSession, miembro_id: int):
    try:
        ...
        query = select(Miembro).options(joinedload(Miembro.rol)).where(Miembro.id == miembro_id)
...
async def listar_miembros_activos_por_hogar(db: AsyncSession, hogar_id: int):
    try:
        ...
        query = select(Miembro).options(joinedload(Miembro.rol)).where(
...
async def actualizar_miembro(db: AsyncSession, miembro_id: int, data: dict):
    try:
        ...
        if "correo_electronico" in data and data["correo_electronico"] != miembro.correo_electronico:
            existe = await db.execute(
                select(Miembro).where(Miembro.correo_electronico == data["correo_electronico"])
            )
            if existe.scalar_one_or_none():
                logger.warning(f"Correo electrónico ya existe: {data['correo_electronico']}")
                raise ValueError("El correo electrónico ya está registrado")
