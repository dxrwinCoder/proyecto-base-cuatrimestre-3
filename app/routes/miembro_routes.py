...
from services.miembro_service import (
    crear_miembro, obtener_miembro, listar_miembros_activos_por_hogar,
    actualizar_miembro, desactivar_miembro
)
...
async def crear_miembro_endpoint(
    ...
    try:
        ...
        if current_user.id_rol != 1 and current_user.id_hogar != miembro.id_hogar:
            logger.warning(f"Intento no autorizado de crear miembro en hogar diferente: {miembro.id_hogar}")
            raise HTTPException(
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno al crear miembro")
...
async def ver_miembro(
    ...
    try:
        ...
        if current_user.id_rol != 1 and current_user.id_hogar != miembro.id_hogar:
            logger.warning(f"Intento no autorizado de ver miembro de otro hogar: {miembro_id}")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def listar_miembros_por_hogar(
    ...
    try:
        ...
        if current_user.id_rol != 1 and current_user.id_hogar != hogar_id:
            logger.warning(f"Intento no autorizado de listar miembros de otro hogar: {hogar_id}")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def actualizar_miembro_endpoint(
    ...
    try:
        ...
        # Verificar permisos
        if current_user.id_rol != 1 and current_user.id_hogar != miembro_actual.id_hogar:
            logger.warning(f"Intento no autorizado de actualizar miembro de otro hogar: {miembro_id}")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="No tiene permiso para actualizar este miembro"
            )
            
        resultado = await actualizar_miembro(db, miembro_id, miembro_update.model_dump(exclude_unset=True))
...
async def eliminar_miembro_endpoint(
    ...
    try:
        ...
        # Verificar permisos
        if current_user.id_rol != 1 and current_user.id_hogar != miembro.id_hogar:
            logger.warning(f"Intento no autorizado de eliminar miembro de otro hogar: {miembro_id}")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
