...
from services.tarea_service import (
    crear_tarea, listar_tareas_por_hogar, obtener_tarea,
    actualizar_tarea, eliminar_tarea_logico, marcar_completada
)
...
async def crear_tarea_endpoint(
    ...
    try:
        ...
        tarea_dict["id_hogar"] = user.id_hogar  # Asegurar que la tarea pertenezca al hogar del usuario
...
async def actualizar_tarea_endpoint(
    ...
    try:
        ...
        if tarea_actual.id_hogar != user.id_hogar:
            logger.warning(f"Intento de actualización no autorizada de tarea: {tarea_id}")
            raise HTTPException(status_code=403, detail="No autorizado")
        
        tarea_actualizada = await actualizar_tarea(db, tarea_id, tarea.model_dump(exclude_unset=True))
...
async def marcar_tarea_completada(
    ...
    try:
        ...
        tarea = await obtener_tarea(db, tarea_id)
        if not tarea:
            logger.warning(f"Tarea no encontrada para marcar como completada: {tarea_id}")
            raise HTTPException(status_code=404, detail="Tarea no encontrada")
        if tarea.id_hogar != user.id_hogar:
            logger.warning(f"Intento no autorizado de marcar tarea como completada: {tarea_id}")
            raise HTTPException(status_code=403, detail="No autorizado")
        ...
        if tarea_actualizada:
            ...
            return tarea_actualizada
        raise HTTPException(status_code=400, detail="No se pudo marcar la tarea como completada")
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error al marcar tarea como completada: {str(e)}")
        raise HTTPException(status_code=500, detail="Error al marcar la tarea como completada")